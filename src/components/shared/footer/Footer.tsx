  ...
  return (
    <footer className='w-full bg-[#0C090A] flex flex-col justify-center py-6 text-xs'>
        ...
        <div className='list-none'>
          ...
          <ul>
            {navigationItems.map((item) =>
              <li key={item.name} className='my-2'><Link href={`/categories/${item.slug}`}>{item.name}</Link></li>
            )}
          </ul>
        </div>
        <div className='list-none'>
      ...
      <div className='self-center flex flex-col justify-center gap-2'>
        <p className='text-white font-roboto text-sm'>© 2024 LearnVerse. All rights reserved.</p>
